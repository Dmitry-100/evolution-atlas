...
// Matches Three.js SphereGeometry's equirectangular UVs: east points toward -Z at Greenwich.
...
export function migrationArc(points: Coordinates[]): GlobePoint[] {
  ...
  for (let segment = 0; segment < points.length - 1; segment++) {
    ...
    for (let step = segment ? 1 : 0; step <= steps; step++) {
      ...
      const weightA =
        angle < 0.0001 ? 1 - t : Math.sin((1 - t) * angle) / Math.sin(angle);
      const weightB =
        angle < 0.0001 ? t : Math.sin(t * angle) / Math.sin(angle);
      result.push(
        a.map(
          (value, i) => (weightA * value + weightB * b[i]) * radius,
        ) as GlobePoint,
      );
    }
  }
