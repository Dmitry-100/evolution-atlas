...
import { HUMAN_MIGRATION_ROUTES, HUMAN_ORIGIN_SITES, type HumanMigrationRoute } from "../../data/humanOrigins";
...
export function FlatMigrationMap({ activeRouteId, activeSiteId, onSiteSelect }: {
  activeRouteId: string;
  activeSiteId: string;
  onSiteSelect: (id: string) => void;
}) {
  return <div className="migration-flat-map">
            ...
            <g className="migration-routes">
              {HUMAN_MIGRATION_ROUTES.map((route) =>
                buildRouteSegments(route).map((segment, index, allSegments) => (
                  <polyline
                    key={`${route.id}-${index}`}
                    className={route.id === activeRouteId ? "migration-route is-active" : "migration-route"}
                    points={segment.join(" ")}
          ...
          {HUMAN_ORIGIN_SITES.map((site) => (
            <button
              key={site.id}
              type="button"
              className={site.id === activeSiteId ? "africa-site is-active" : "africa-site"}
