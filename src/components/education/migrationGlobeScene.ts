...
function earthTexture() {
  ...
  wash.addColorStop(0, "#495c57");
  wash.addColorStop(0.28, "#52685b");
  wash.addColorStop(0.48, "#657560");
  wash.addColorStop(0.7, "#3d6157");
  wash.addColorStop(1, "#657c71");
  ctx.fillStyle = wash;
...
export function createMigrationGlobe(
  options: Options,
): MigrationGlobeController {
  ...
  canvas.setAttribute(
    "aria-label",
    "Трёхмерный глобус расселения Homo sapiens. Поворот — перетаскиванием или стрелками, масштаб — клавишами плюс и минус.",
  );
  canvas.tabIndex = 0;
  host.prepend(canvas);
  ...
  // Leave ordinary scroll to the page; explicit buttons control zoom.
  ...
  scene.add(earth);
  scene.add(new THREE.AmbientLight("#a4babb", 0.9));
  const keyLight = new THREE.DirectionalLight("#fff0cb", 2.4);
  scene.add(keyLight);
  ...
  for (let latitude = -60; latitude <= 60; latitude += 30) {
    const points = Array.from(
      { length: 181 },
      (_, i) =>
        new THREE.Vector3(...globePoint([-180 + i * 2, latitude], 1.002)),
    );
    scene.add(
      new THREE.Line(
  ...
  for (let longitude = -180; longitude < 180; longitude += 30) {
    const points = Array.from(
      { length: 91 },
      (_, i) =>
        new THREE.Vector3(...globePoint([longitude, -90 + i * 2], 1.002)),
    );
    scene.add(
      new THREE.Line(
        new THREE.BufferGeometry().setFromPoints(points),
  ...
  const routeObjects = HUMAN_MIGRATION_ROUTES.map((route) => {
    const points = migrationArc(route.points).map(
    ...
    const line = new THREE.Mesh(
      new THREE.TubeGeometry(curve, points.length * 2, 0.0032, 5, false),
      material,
    );
  ...
  const sites = HUMAN_ORIGIN_SITES.map((site) => {
    const position = new THREE.Vector3(
      ...globePoint([site.longitude, site.latitude], 1.015),
    );
    const dot = new THREE.Mesh(
  ...
  const traveler = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: "#fff7dd" }),
  );
  scene.add(traveler);
  ...
  function focus(coordinates: [number, number]) {
    targetPosition = new THREE.Vector3(
      ...globePoint(coordinates, camera.position.length()),
    );
    if (motion.matches) {
  ...
  function updateLabels() {
    ...
    sites.forEach(({ site, position }, index) => {
      ...
      const front =
        position.clone().normalize().dot(view) >
        1 / camera.position.length() + 0.035;
      const onScreen =
        front &&
        x > 20 &&
        x < width - 20 &&
        y > 72 &&
        y < height - (width < 420 ? 95 : 56);
  ...
  function render(time: number) {
    ...
    if (targetPosition) {
      ...
      camera.position
        .applyQuaternion(
          new THREE.Quaternion().slerp(rotation, 1 - Math.exp(-delta * 5)),
        )
        .setLength(distance);
    ...
    const selected = routeObjects.find(
      ({ route }) => route.id === activeRouteId,
    );
    if (selected && traveler.visible)
      traveler.position.copy(selected.curve.getPointAt((travelTime / 7) % 1));
    updateLabels();
  ...
  function resize() {
    ...
    camera.fov = width < 420 ? 49 : width < 500 ? 43 : 36;
  ...
  function zoom(factor: number) {
    ...
    camera.position.setLength(
      THREE.MathUtils.clamp(camera.position.length() * factor, 2.65, 5),
    );
    controls.update();
    requestRender();
  }
