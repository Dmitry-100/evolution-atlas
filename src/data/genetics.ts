...
export const GENOME_COMPARISONS: GenomeComparison[] = [
  {
    ...
    cautionRu:
      "99,9% обычно относится только к однонуклеотидным заменам; структурные варианты и повторы тоже важны.",
  ...
  },
  {
    ...
    cautionRu:
      "Это не значит, что человек 'почти шимпанзе': важны и отличия, и регуляция, и то, где именно меняется последовательность.",
...
export const GENETICS_EVIDENCE: GeneticsEvidence[] = [
  {
    id: "shared-code",
    titleRu: "Почти общий язык кодонов",
    shortRu:
      "ДНК и РНК читаются тройками букв: 64 кодона задают аминокислоты или стоп-сигналы. Генетический код в основном общий, но редкие варианты показывают, что это правило не абсолютно.",
    whyItMattersRu:
  ...
  },
  {
    ...
    whyItMattersRu:
      "Это след конкретного события в человеческой линии после ее расхождения с другими человекообразными.",
